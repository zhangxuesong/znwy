...
$auth = sha1($ps . $log['uniacid'] . $_W['config']['setting']['authkey']);
if($auth != $_GPC['auth']) {
	exit('参数传输错误.');
}
...
if($property['ispay']==1 && !empty($property['submerchid'])){
	if (intval($wechat['switch']) == 1) {
		$wechat['sub_mch_id'] = trim($property['submerchid']);
		$wechat['signkey'] = $sysset['merchsecret'];
		$wechat['mchid'] = $sysset['servicemerchid'];
		$wOpt = wechat_build($params, $wechat);	
	}
	else{
		$this->mymsg('error','支付失败','微擎中没有开启微信支付.','');	
		exit;
	}	
}
else { 
...
if (is_error($wOpt)) {
	...
	exit("抱歉，发起支付失败，具体原因为：“{$wOpt['errno']}:{$wOpt['message']}”。请及时联系站点管理员。");
}
