...
error_reporting(0);
	...
	pdo_update('rhinfo_zyxq_version',array('secretkey'=>date('YmdHis')));
	
	exit;
	
	load()->web('common');
	$_W['uniacid'] = $_W['weid'] = intval($_POST['rhinfo_i']);
	...
	$region_id = $_GET['rhinfo_rid'];
	$sql = "select * from ".tablename("rhinfo_zyxq_region").' where weid = :weid and id = :rid ';
	$region = pdo_fetch($sql,array(':weid'=>$_W['uniacid'],':rid'=>$region_id));
