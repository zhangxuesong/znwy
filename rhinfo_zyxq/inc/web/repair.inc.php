...
if ($operation == 'list') {
    ...
    while (!($m >= count($regions))) {
        ...
        $sql = 'select id,title from ' . tablename('rhinfo_zyxq_category') . ' where weid = :weid and rid = :rid and type=2';
    ...
    if ($total > 0) {
        ...
        while (!($k >= count($data))) {
            ...
            if (!empty($team)) {
                ...
                $sql = 'select * from ' . tablename('rhinfo_zyxq_repair_record') . ' where weid = :weid and rid = :rid and (openid<>:openid or uid<>:uid)';
            ...
            if ($region['finishdays'] > 0 && $data[$k]['status'] == 3) {
                ...
                if ($days > $region['finishdays']) {
                    ...
                    $postdata = array('first' => array('value' => empty($region['tplnoticefirst']) ? $region['title'] : $region['tplnoticefirst']), 'keyword1' => array('value' => '您的报修工单已自动结案', 'color' => $topcolor), 'keyword2' => array('value' => date('Y-m-d H:i', TIMESTAMP), 'color' => $textcolor), 'keyword3' => array('value' => $data[$k]['address'] . '，报修内容：' . $data[$k]['content'], 'color' => $textcolor), 'remark' => array('value' => '快去给该工单处理做个评价吧，谢谢！'));
                    $url = $this->createMobileurl('steward', array('op' => 'repaircomment', 'repairid' => $data[$k]['id']));
        ...
        if ($_GPC['export'] != '') {
            $filter = array('id' => 'ID', 'regionname' => '所属主体', 'address' => '房产', 'catename' => '报修类别', 'content' => '报修内容', 'nickname' => '报修人', 'getrealname' => '处理人', 'statustxt' => '状态', 'reporttimes' => '超时次数', 'ctime' => '报修时间');
            export_excel($data, $filter, '报修明细');
            exit(0);
        }
    ...
    $sql = 'select * from ' . tablename('rhinfo_zyxq_repair_record') . ' where weid = :weid and rid = :rid';
    ...
    $id = intval($_GPC['id']);
    $glue = 'AND';
    $result = pdo_delete('rhinfo_zyxq_repair_record', array('id' => $id, 'weid' => $mywe['weid']), 'AND');
    if (!empty($result)) {
        echo 'ok';
    } else {
        echo '删除失败!';
    ...
    if ($_W['ispost']) {
        ...
        $str .= !empty($room) ? '-' . $room : '';
        $data['address'] = $str;
        pdo_insert('rhinfo_zyxq_repair', $data);
        $id = pdo_insertid();
    ...
    while (!($k >= count($myproperty))) {
        ...
        while (!($m >= count($regions))) {
            ...
            $buildings = pdo_fetchall($sql, array(':weid' => $mywe['weid'], ':rid' => $regions[$m]['id']));
            $mybuilding[$regions[$m]['id']] = $buildings;
            $n = 0;
            while (!($n >= count($buildings))) {
                $sql = 'select id,title from ' . tablename('rhinfo_zyxq_unit') . ' where weid = :weid and rid = :rid and bid = :bid';
                $units = pdo_fetchall($sql, array(':weid' => $mywe['weid'], ':rid' => $regions[$m]['id'], ':bid' => $buildings[$n]['id']));
                $myunit[$buildings[$n]['id']] = $units;
                $j = 0;
                while (!($j >= count($units))) {
                    $sql = 'select id,title from ' . tablename('rhinfo_zyxq_room') . ' where weid = :weid and rid = :rid and bid = :bid and tid=:tid';
                    $rooms = pdo_fetchall($sql, array(':weid' => $mywe['weid'], ':rid' => $regions[$m]['id'], ':bid' => $buildings[$n]['id'], ':tid' => $units[$j]['id']));
    ...
    if ($_W['isajax']) {
        ...
        while (!($k >= count($send_teams))) {
            ...
            $url = $this->my_mobileurl($this->createMobileurl('steward', array('op' => 'repairtrack', 'id' => $id)));
            ...
            if ($region['repairverify'] == 1 && !empty($this->syscfg['repairid'])) {
                ...
                $this->send_sms($this->syscfg['smstype'], $send_teams[$k]['mobile'], $this->syscfg['repairid'], array('content' => $item['content'], 'phone' => $member['mobile']));
            }
            ($i += 1) + -1;
            ($k += 1) + -1;
        }
    ...
    $myret = 1;
    $id = intval($_GPC['id']);
    $condition .= ' and repairid = :repairid';
    $params[':repairid'] = $id;
    $sql = 'SELECT COUNT(*) FROM ' . tablename('rhinfo_zyxq_repair_comment') . ' where ' . $condition;
    $total = pdo_fetchcolumn($sql, $params);
    if ($total > 0) {
        $sql = 'select * from ' . tablename('rhinfo_zyxq_repair_comment') . ' where ' . $condition . " ORDER BY\r\n\t\t\t\t\t `ID` ASC " . $limit;
        $data = pdo_fetchall($sql, $params);
        $pager = pagination($total, $pindex, $psize);
    }
    ...
    $id = intval($_GPC['id']);
    $glue = 'AND';
    $result = pdo_delete('rhinfo_zyxq_repair_comment', array('id' => $id, 'weid' => $mywe['weid']), 'AND');
    if (!empty($result)) {
        echo 'ok';
    } else {
        echo '删除失败!';
    ...
    if ($_W['ispost']) {
        ...
        if ($res) {
            pdo_update('rhinfo_zyxq_repair', array('status' => $_GPC['status'], 'lasttime' => TIMESTAMP), array('weid' => $mywe['weid'], 'id' => $id));
            $topcolor = empty($this->syscfg['topcolor']) ? '#FF683F' : $this->syscfg['topcolor'];
            $textcolor = empty($this->syscfg['textcolor']) ? '#000' : $this->syscfg['textcolor'];
            $repairtitle = pdo_fetchcolumn('select title from ' . tablename('rhinfo_zyxq_category') . ' where id = :cid and weid = :weid', array(':cid' => $item['cid'], ':weid' => $mywe['weid']));
            ...
            $postdata = array('first' => array('value' => '尊敬的业主，您的报修已回复', 'color' => $topcolor), 'keyword1' => array('value' => $item['address'], 'color' => $textcolor), 'keyword2' => array('value' => $repairtitle, 'color' => $textcolor), 'keyword3' => array('value' => $serviceuser, 'color' => $textcolor), 'keyword4' => array('value' => date('Y-m-d H:i', TIMESTAMP), 'color' => $textcolor), 'remark' => array('value' => '如有疑问，请尽快回复工单，谢谢！'));
            $url = $this->createMobileurl('steward', array('op' => 'repairtrack', 'id' => $id));
            $url = $this->my_mobileurl($url);
            if (!empty($this->syscfg['tplid3'])) {
                $this->send_mysendtplnotice($item['openid'], $this->syscfg['tplid3'], $postdata, $url, $topcolor);
            } else {
                $this->send_mycusnewsmsg('尊敬的业主，您的报修已处理', '如有疑问，请尽快回复工单，谢谢！', $url, '', $item['openid']);
            }
        }
        header('Location:' . $this->createWeburl($mydo, array('op' => 'list')) . $mywe['direct']);
        exit(0);
